...
function update(dt, fireMode, shiftHeld)
	...
	elseif self.useTimer < 5.5 then
		...
		world.spawnVehicle( "spovxeronious", { position[1], position[2] + 1.5 }, { driver = entity.id(), settings = {} } )

		world.spawnItem("xeroniouscodex-codex", position, 1)

		player.radioMessage({
		messageId = "xeroniouspotion", unique = false,
		text = "It seems that the potion you just drank transformed you into a kaiju of some sort. Scans indicate that the ^green;[F]^reset;, ^green;[G]^reset;, and ^green;[H]^reset; buttons will be helpful in this form. ^#555;(You may have to go into settings and bind the ^#711;Tech Action 2^#555; and ^#711;3^#555; keys first.)^reset; If you need further instructions on your capabilities I suggest buying a guide from that potion dealer."
		}, 1)
